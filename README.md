# FTH 订单自动化助手 (FTH Order Automation Assistant) v4.0
...
### v4.0 系统架构与稳定性升级
- **TypeScript 全面重构**：重定义了核心数据模型（FuluProduct），消除了全系统的类型报错，极大增强了代码的可维护性。
- **构建配置优化**：全面支持 ESM 环境下的路径解析，修复了 Vite 插件中的隐式 any 类型报错，实现了生产环境的一键成功构建。
- **数据归一化**：新增 IndexedDB 自动迁移逻辑，老版本数据在加载时会自动归一化为新版对象结构，确保平滑升级不丢数据。
- **UI 交互增强**：优化了设置页面的货品管理交互，支持更精准的条码、规格及单价配置。
